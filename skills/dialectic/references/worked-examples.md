...
**When to read:** Before your first orchestration, or when you need to calibrate prompt quality. Study these to understand the level of specificity, framing correction, and structural craft the skill requires.
...
- Monk A: "Search for Vinxi and Nitro as open infrastructure primitives. Search for structural arguments about how Vercel's business model shapes Next.js's architectural decisions."
...
**Ontological question:** "What is the proper relationship between a framework, the infrastructure it runs on, and the business interests that fund its development?"
...
**No external research needed.** The briefing was built entirely from the elenctic interview.
...
Each synthesis opens doors to domains the previous round couldn't see.

---

## What Makes Good Prompts Work

1. **Targeted research directives.** Not "research TanStack Start" but "search for TanStack Router's type-safety model specifically."
2. **Framing corrections preempt degenerate dialectics.** "TanStack Start IS a framework — your argument is NOT that it's more modular."
3. **Ontological questions force depth.** "What IS the proper relationship between X and Y?"
...
5. **Opponent restatement prevents shadowboxing.** Monk B is warned: "Your opponent's argument is NOT the naive take."
6. **Parallel structure enables comparison.** Both prompts follow the same shape so outputs are structurally comparable.
